...
# line graph for rate per 100,000 
line_per100k = function(cases_week,start_point="Date") {
  g1 = ggplot(cases_week, aes(x = weeks, y = outcome, colour = region, group =1,
                              text = paste0("País:", region, "\n", "Tasa", outcome))) +
    geom_line(alpha=1) + geom_point(size = 1, alpha = 1) +
    xlab("Semanas") +
    ylab("Casos por 100,000 hab.") + theme_bw() + 
...
# create subset for countries with at least 150 cases
cv_today_150 = subset(cv_today, cases>=150)
...
cases_week <- subset(cv_cases, !is.na(continent_level)) %>% group_by(country,weeks, global_level, continent_level) %>%
  summarise(cases=max(cases),new_cases=sum(new_cases), deaths=max(deaths),new_deaths=sum(new_deaths), days_since_case100=max(days_since_case100), days_since_death10=max(days_since_death10), population=last(population))

cases_week$per100k = as.numeric(format(round(cases_week$new_cases/(cases_week$population/100000),1),nsmall=1))
...
bins = c(0,1,10,50,100,500,1000,1500)
                      ...
                      sidebarLayout(
                        sidebarPanel(
                          ...
                          pickerInput("region_select", "País/Región:",
                                      ...
                                      selected = c("El Salvador", "Guatemala", "USA","Nicaragua", "Mexico" ,"Spain", "Honduras", "Costa Rica", "Panama", "Mainland China"),
                                      multiple = TRUE),
                        ...
                        mainPanel(
                          tabsetPanel(
                            ...
                            tabPanel("Casos por 100,000",
                                     fluidRow(
                                       column(width = 6.5, class="well",
                                              h4("Tasa acumulada por 100,000 hab."),
                                              plotlyOutput("country100k")
                                       ),
                                       column(width = 6.5, class="well",
                                              h4("Tasa semanal por 100,000 hab."),
                                              plotlyOutput("line100k")
                                       )
                                     )
                            ),
...
server = function(input, output, session) {
  ...
  # reactive for line chart
  country_reactive_db_100 = reactive({
    if (input$level_select=="Global") {
      print("Gráfica no disponible para este nivel.") 
    }
    if (input$level_select=="Continent") {
    ...
    if (input$level_select=="Country") {
      cases_week$region = cases_week$country
      
    }
    if (input$outcome_select=="Cases") {
      cases_week$outcome = cases_week$per100k
    }
    if (input$outcome_select=="Deaths") {
      print("Gráfica no disponible para este nivel.") 
    }
    cases_week %>% filter(region %in% input$region_select)
  })
  ...
  # country line chart per 100k
  output$line100k <- renderPlotly({
    line_per100k(country_reactive_db_100(), start_point=input$start_date)
  })
